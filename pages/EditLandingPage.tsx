...
const EditLandingPage: React.FC = () => {
    ...
    return (
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <h1 className="text-3xl font-bold text-text mb-6">Edit a Card</h1>
            <p className="text-muted mb-8">Select a card to make changes.</p>
            ...
            {cards.length === 0 ? (
                <div className="text-center py-20 bg-card rounded-lg shadow-sm border border-slate-200">
                    ...
                    <p className="text-muted mt-2">Create a card first to edit it.</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {cards.map(card => (
                        <Link to={`/edit/${card.id}`} key={card.id} className="block bg-card rounded-lg shadow-sm border border-slate-200 overflow-hidden hover:shadow-md hover:border-primary transition">
                            <div className="p-6">
